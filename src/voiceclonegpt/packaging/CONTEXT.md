...
3. If the bundle already exists, refuse with `BundleExistsError` unless
   `replace=True`.
4. Write `Contents/Info.plist` — including the `VCGBuiltBy` marker — and
...
exec python3 -m voiceclonegpt.studio_app
```
...
| `VCGBuiltBy == "voiceclonegpt.packaging.app_bundle"` | **an `Info.plist` alone proves nothing — every macOS app has one.** Only a bundle carrying this tool's marker may be removed |
