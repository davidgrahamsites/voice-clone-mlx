...
**Human check:** `PYTHONPATH=src python3 -m voiceclonemlx.studio_app` opens a
...
- `__main__.py` — `python3 -m voiceclonemlx.studio_app`.
...
- Bus integration attaches at `shared.integration_seam.set_event_sink`. This
  ...
  never imports `voiceclonemlx.bus` directly.
...
VOICECLONEMLX_RUN_UI_TESTS=1 python3 -m pytest tests/voice_studio/test_studio_app.py   # UI tests run
```
