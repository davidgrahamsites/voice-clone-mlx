...
**Human check:** `PYTHONPATH=src python3 -m voiceclonemlx.reader_app` opens a
...
- `__main__.py` — `python3 -m voiceclonemlx.reader_app`.
...
`voiceclonemlx.shared.roundtrip` **does not exist in this checkout** — it lives
...
`VOICECLONEMLX_RUN_UI_TESTS=1`. Splitting the 480-line test module into 291 +
...
VOICECLONEMLX_RUN_UI_TESTS=1 python3 -m pytest tests/voice_reader/test_reader_app.py   # UI tests run
```

Without `VOICECLONEMLX_RUN_UI_TESTS=1` the real-window tests skip **before Tk is
...
ERROR — ModuleNotFoundError: No module named
        'voiceclonemlx.reader_app.manifest_roundtrip'
