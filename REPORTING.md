...
- Every status update begins with the current estimated percentage of the
  entire project that is finished and the current system timestamp, including
  time zone. Keep the estimate honest and change it only when verified work
  changes the remaining path to the real voice-model round trip.
